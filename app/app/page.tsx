  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        {nivel === "ebook" ? (
          <p className="text-rest-text-muted mt-1">Tu ebook del Método R.E.S.T. te espera</p>
        ) : (
      ...
      {nivel === "ebook" && (
        <div>
          <Link
            href="/app/ebook"
            className="group flex items-center gap-4 p-5 rounded-2xl glass-card card-glow glow-accent-sm hover:!bg-[rgba(0,229,160,0.15)] hover:scale-[1.01] transition-all"
          >
            <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-rest-accent to-emerald-400 flex items-center justify-center">
              <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
              </svg>
            </div>
            <div className="flex-1">
              <p className="font-medium text-white">Leer mi ebook</p>
              <p className="text-rest-text-muted text-xs mt-0.5">Continuar la lectura del Método R.E.S.T.</p>
            </div>
      ...
      {/* Aviso de método completo para nivel ebook */}
      {nivel === "ebook" && (
        <div className="p-5 rounded-2xl bg-gradient-to-r from-rest-accent/10 to-rest-accent/5 text-center">
      ...
      {/* Contenido del metodo: funcional para "completo", difuminado y bloqueado para "ebook" */}
      <div className={nivel === "ebook" ? "relative select-none" : ""}>
        {nivel === "ebook" && (
          <div className="absolute inset-0 z-10" style={{ cursor: "not-allowed" }} aria-hidden="true" />
        )}
        <div className={nivel === "ebook" ? "opacity-20 pointer-events-none select-none space-y-6" : "space-y-6"} style={nivel === "ebook" ? { filter: "blur(10px)" } : undefined}>
      {/* Evaluation banner */}
      {evalDue && (
      ...
      {/* Quick action — Ebook (solo para nivel completo; ebook lo tiene arriba) */}
      {nivel === "completo" && (
      <div>
